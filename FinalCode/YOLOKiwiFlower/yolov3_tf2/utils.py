...
def flower_position(fig, center, center_pos_box, diag_lenght):
    # Posición X:
    coord_y = int((center[0] - center_pos_box[0]) * 0.6) / 1.e3
    # Posición Y:
    coord_z = int((center[1] - center_pos_box[1]) * 0.6) / 1.e3
    # Posición Z:
...
def draw_outputs(img, outputs, class_names, fig):
    # colors = ((np.array(color_palette("hls", 80)) * 255)).astype(np.uint8) # Reemplazar
